            ...
            <div className="scrollbar-subtle flex-1 overflow-y-auto px-6 py-4 sm:px-8 sm:py-5">
              ...
              {decision && !decisionActed && (
                <div className="mb-7 flex flex-wrap gap-2">
                  <button type="button" onClick={() => { setDecisionActed(true); setMessages((c) => [...c, { from: "agent", body: "Recorded. Nothing was applied to your cloud — 0 cloud writes. The fix stays modeled on the branch until you promote it yourself.", time: "now" }]); }} className="rounded-full bg-[#c7f36b] px-4 py-2 text-xs font-medium text-[#141616] transition hover:brightness-110">Approve fix</button>
                  <button type="button" onClick={() => { setDecisionActed(true); setMessages((c) => [...c, { from: "agent", body: "Kept as-is. Branch discarded — nothing changed.", time: "now" }]); }} className="rounded-full border border-white/15 px-4 py-2 text-xs text-white/75 transition hover:bg-white/10">Keep as-is</button>
